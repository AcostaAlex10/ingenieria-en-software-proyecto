...
router.get('/:planId/avances/resumen', async (req, res, next) => {
  try {
    ...
    // Esperado calculado a hoy según etapas; fallback al campo estático.
    const esperado = await calcularEsperadoHoy(planId, plan.avance_esperado_total);

    res.json({
      avance_esperado: esperado,
      avance_real: avanceReal,
      desvio_pp: parseFloat((avanceReal - esperado).toFixed(2)),
